...
public class AxolotlTools extends Module {
    ...
    private <T extends LivingEntity> boolean tryInteractMobFull(T entity, Item bucketType) {
        ...
        for (int n = 0; n < ((PlayerInventoryAccessor) mc.player.getInventory()).getMain().size(); n++) {
            ...
            if (n != ((PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot()) {
                if (n < 9) InvUtils.swap(n, false);
                else InvUtils.move().from(n).to(((PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot());
            }
    ...
    @EventHandler private void onTick(TickEvent.Pre event) {
        ...
        ItemStack current = mc.player.getInventory().getStack(((bep.hax.mixin.accessor.PlayerInventoryAccessor) mc.player.getInventory()).getSelectedSlot());
